...
| 项目 | 你的填写 |
| :--- | :------- |
| 日志中 `IN=` 字段的值 | Network-fw-wan|
| 日志中 `OUT=` 字段的值 |net-fw-dm2 |
...
> 答：日志中的 `DST=10.0.3.2` 而不是 `203.0.113.1` 说明 LOG 规则是在 DNAT 完成地址转换之后触发的，即位于 FORWARD 链，因为 PREROUTING 链已经将原始目标地址 `203.0.113.1` 改写为 `10.0.3.2`，后续的 FORWARD 链只能看到改写后的地址；如果要记录原始目标地址，应该把 LOG 规则放在 `nat` 表的 `PREROUTING` 链中，并且要在 DNAT 规则之前执行，这样才能在地址转换前捕获原始的 `203.0.113.1` 目标地址。
