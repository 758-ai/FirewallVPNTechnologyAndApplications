...
| TTL（若可见）                |         801 (13 分钟 21 秒)       |

**嵌入截图：**

![alt text](dns_resp.png)
...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS 的作用：域名系统（DNS）负责将人类可读的域名（如 www.yxnu.edu.cn）转换为网络设备可识别的 IP 地址（如 61.166.32.242），实现域名与 IP 地址的映射。
   需要先解析的原因：计算机网络通信依赖 IP 地址寻址，无法直接识别域名，因此必须先通过 DNS 解析获取目标网站的 IP 地址，才能建立 TCP 连接并访问网站。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：一致性：一致，本次实验中请求包和响应包的 Transaction ID 均为 0x3c7e。
   作用：作为本次 DNS 查询的唯一标识符，用于客户端匹配对应的请求与响应，避免网络中多个 DNS 报文混淆。


3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录（Address Record）是 DNS 资源记录的一种，表示将域名映射到对应的 IPv4 地址，是最基础的地址解析记录类型。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：默认协议与端口：UDP 协议，端口号 53。
改用 TCP 的场景：
(1)当 DNS 响应报文长度超过 512 字节时（UDP 报文最大限制）；
(2)进行区域传输（Zone Transfer）时，需可靠传输保证数据完整性；
(3)部分安全场景（如 DNSSEC）要求使用 TCP 建立连接。


5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：(1)不支持 IPv6：仅能解析 IPv4 地址，无法适配 IPv6 网络环境，而现代网络已逐步向 IPv6 过渡；
(2)线程不安全：返回的 hostent 结构体是静态分配的，多线程调用时会存在数据竞争和覆盖问题；
(3)功能局限：无法获取更详细的解析信息，且错误处理机制简陋，现代替代接口为 getaddrinfo()。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：通常说明该网站采用了负载均衡或高可用架构：
多个 IP 对应多台服务器，可将用户请求分发到不同节点，避免单点故障，提升服务稳定性；
也可能是基于地理位置的智能 DNS 解析，将用户导向最近的服务器，降低访问延迟。
